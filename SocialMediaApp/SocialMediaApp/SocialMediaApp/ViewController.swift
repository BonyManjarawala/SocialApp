...
class ViewController: UIViewController {
    ...
    private let bgimage: UIImageView = {
        ...
        bgimage.image = UIImage(named: "soc")
        return bgimage
    }()
    ...
    private let vcbtn : UIButton = {
        let vcbtn = UIButton()
        vcbtn.setTitle("Register", for: .normal)
        vcbtn.backgroundColor = .black
        vcbtn.addTarget(self, action: #selector(redirect), for: .touchUpInside)
        vcbtn.layer.cornerRadius = 50
        return vcbtn
    } ()
    @objc func redirect()
    {
    ...
    override func viewDidLayoutSubviews() {
        ...
        vcbtn.frame = CGRect(x: 20, y: vcbutton.bottom + 20,width: view.width - 40, height: 40)
